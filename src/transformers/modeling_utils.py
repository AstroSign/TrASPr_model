            ...
            try:
                ...
                if config.model_type != 'bert':
                    for i in state_dict:
                        ...
                        old_vocab_size = kmer_old_vocab_size[config.kmer]
                        # vocab_size = config.vocab_size
                        # old_vocab_size = 4101         #6-mer
                        # old_vocab_size = 1029         #5-mer
                        # old_vocab_size = 69         #3-mer
                        #old_vocab_size = 261
                        # old_vocab_size = 4125 #for refinetune only
                        if old_vocab_size in state_dict[i].shape:
                            if len(state_dict[i].shape) == 1:
                                temp = torch.rand(vocab_size)
                                temp[:old_vocab_size] = state_dict[i]
                                state_dict[i] = temp
                                print(i,state_dict[i].shape)
                            elif len(state_dict[i].shape) == 2:
                                temp = torch.rand((vocab_size,state_dict[i].shape[1]))
                                temp[:old_vocab_size,:] = state_dict[i]
                                state_dict[i] = temp
                                print(i,state_dict[i].shape)
                            else:
                                print("other shape:",i,state_dict[i].shape)
                    if config.tf_mode == "two_tf_twoFC":
                        num_models = 2
                    elif config.model_type == "dnamultitransalt":
                        num_models = 3
                    elif config.model_type == "dnamultitransaltfull":
                        num_models = 6
                    else:
                        num_models = 4
                    # import pdb; pdb.set_trace()
                    if config.model_type == "dnamultitrans" or config.model_type == "dnamultitransalt" or config.model_type == "dnamultitransaltfull" or config.tf_mode == "two_tf_twoFC":
                        state_list = list(state_dict.keys())
                        if not any(s.startswith("bert.0") for s in state_list):
                            for s in state_list:
                                if s.startswith("bert"):
                                    temp = s.split('.')
                                    for i in range(num_models):
                                        new_s = temp[0] + '.' + str(i) + '.' + '.'.join(temp[1:])
                                        state_dict[new_s] = state_dict[s]
                                    del state_dict[s]
            except Exception:
                raise OSError(
